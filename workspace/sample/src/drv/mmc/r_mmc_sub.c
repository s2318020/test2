...
{
	gMmc_Media[SlotNo] = MMC_CARD_UNDETECT;				/* Card is not found setting					0*/
	gMmc_WP[SlotNo]    = MMC_W_PROTECT_SOFT;			/* Write-protection setting						2*/
...
int16_t R_mmc_Init_Media(uint8_t SlotNo)
{
	...
	/* CRC option setting */
	Ret = mmc_Cmd59(R_mmc_ExchgLong(MMC_CRC_OP_ON)); //crc
	if (Ret < MMC_OK)
	{
		R_mtl_Log(MMC_LOG_ERR, 0, MMC_FID, __LINE__);
		return Ret;
...
int16_t R_mmc_Write_Start(uint8_t SlotNo, uint32_t BlkNo)
{
	...
	DataAddr = BlkNo * gMmc_AddrRev[SlotNo];  //= BlkNo * 512
	for (RtyCnt = 128; RtyCnt != 0; RtyCnt--)
	{
