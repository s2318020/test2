...
STATIC const uint16_t Taac_Unit_Tbl[8]		= {0  , 0   , 0    , 10000, 1000, 100  , 10 , 1    };	/* 10ms * 1/n to n. but nano second order is defined to 0. */
STATIC const uint8_t  Taac_Unit_div			= 100;					/* common divisor value */

STATIC const uint8_t  Taac_Value_Tbl[16]	= {0, 10, 12, 13, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60, 70, 80};	//[0] is resserved. unit = 0.1
...
int16_t R_mmc_Reset_Mmc(uint8_t SlotNo)
{
	...
	R_mmc_Sync_Enable(0, MMC_UBRG_D_TRANSFER);			/* Data Transfer mode change 鮑率					 */
